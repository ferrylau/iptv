/*
 * ddxq_cookie_catcher.js
 * 功能：当打开叮咚买菜时, 自动抓取签到、农场等功能的会话信息。
 * 运行环境: Shadowrocket / Surge / Loon / Stash / Quantumult X
 *
 * 最后更新: 2026-02-09
 */
const scriptName = '叮咚买菜-通用Cookie获取';

// --- 签到相关配置 ---
const checkinUrlRegex = /^https?:\/\/maicai\.api\.ddxq\.mobi\/point\/home\?api_version/;
const checkinCookieKey = 'dingdongmaicai_checkin_cookie_v1';
const checkinBodyKey = 'dingdongmaicai_checkin_body_v1';
  ...
  return new Promise((resolve, reject) => {
    $.http.get({
      ...
      if (obj.code === 0) {
        ...
        const msg = `获取UserId失败${JSON.stringify(resp)}`;
        $.logger.warning(msg);
        reject(msg);
      }
    }).catch(err => {
      const msg = `获取UserId异常${err}`;
      $.logger.error(msg);
      reject(msg);
    })
  })
